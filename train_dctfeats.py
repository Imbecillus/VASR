...
while not abort:
    ...
    for xb, yb in train_dl:
        ...
        if bidirectional:
            forward = prediction[:,:len(truth_table)]
            backward = prediction[:,len(truth_table):]
    ...
    if perform_epoch_evaluation and ((epoch) % eval_every == 0 or epoch == 1):
        ...
        train_acc, train_classes = helpers.lstm_evaluate(model, dataset_epochval, truth_table, ground_truth='index', device=device, limit=epoch_evaluation_size_limit, dct_feats=True, bidirectional=bidirectional)
        ...
        valid_acc = ''
        valid_acc, valid_classes =  helpers.lstm_evaluate(model, validationset_epochval, truth_table, ground_truth='index', device=device, limit=epoch_evaluation_size_limit, dct_feats=True, bidirectional=bidirectional)
...
acc, _, confusion_dict = helpers.lstm_evaluate(model, validationset, truth_table, 'index', device, print_confusion_matrix=True, dct_feats=True, bidirectional=bidirectional)
