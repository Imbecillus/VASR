...
class Net(torch.nn.Module):
    def __init__(self, channels, target_size, cnn_dense_units, cnn_filters, dropout_rate, device):
        ...
        self.layers = []
        self.layers.append(residual_block(cnn_filters[0], cnn_filters[0], (3, 3), 1, device, project_shortcut=False, skip_bn=True))
        for ix, num_filters in enumerate(cnn_filters[1:]):
            self.layers.append(residual_block(cnn_filters[ix], num_filters, (3, 3), 2, device, project_shortcut=True).to(device))
